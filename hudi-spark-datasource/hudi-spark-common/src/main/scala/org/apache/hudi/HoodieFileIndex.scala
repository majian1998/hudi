...
case class HoodieFileIndex(spark: SparkSession,
  ...
  : Seq[(Option[BaseHoodieTableFileIndex.PartitionPath], Seq[FileSlice])] = {
    ...
    val prunedFileSlices: Set[String] = if (!includeLogFiles) {
      ...
      prunedPartitionsAndFileSlices
        ...
        .flatMap(fileSlice => {
          val baseFileName = fileSlice.getBaseFile.get().getFileName
          val logFileNames = fileSlice.getLogFiles.iterator().asScala.toSeq.map(_.getFileName)
          Seq(baseFileName) ++ logFileNames
        })
        .toSet
    }
